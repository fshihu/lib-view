...
<div class="wrapper" ref="wrapper">
  <textarea ref="textarea" v-model="textarea_val" @change="change" class="ant-input" id="textarea_ipt_line" v-bind="$attrs" :placeholder="placeholder"/>
</div>
</template>
...
export default {
  name: "xb-input-textarea-line-num",
  props:{
      value:'',
    placeholder:{
        type:String,
      default:'第一行请输入标题，例:国家/省份/城市/区县\n中国/四川省/成都市/武侯区'
    }
  },
  data(){
      return {
  ...
  watch:{
    value:{
      handler(){
        this.textarea_val = this.value;
      },
      immediate:true,
    }
  },
  mounted() {
    TLN.append_line_numbers('textarea_ipt_line');
  },
  methods:{
    change(){
      this.$emit('input',this.textarea_val);
    },
    setErr(line){
      this.$refs.wrapper.querySelectorAll('.tln-line').forEach(item =>{
        item.classList.remove('error');
      });
      this.$refs.wrapper.querySelectorAll('.tln-line')[line].classList.add('error')
    }
  }
}
</script>
...
.tln-wrapper {
  width: 3em;
  padding: 0.6275em 0.3125em 2.1875em;
  height: calc(100% - 4px);
  word-break: break-all;
  overflow: hidden;
